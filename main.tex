...
\begin{figure*}[t]
\centering
\includegraphics[width=0.6\textwidth]{figures/crosslayer_architecture1.png}
...
```
\STATE Begin packet transmission
\STATE Monitor physical layer signal characteristics

\IF{abnormal signal variation detected}
    \STATE Flag node as suspicious
    \STATE Log physical layer anomaly
\ELSE
    \STATE Continue normal transmission
\ENDIF

\STATE Extract network traffic features
\STATE Apply anomaly detection model

\IF{traffic classified as malicious}
    \STATE Mark node as attacker
    \STATE Update intrusion log
\ELSE
    \STATE Update normal traffic profile
\ENDIF

\STATE Calculate node trust value $T_i$

\IF{$T_i < Threshold$}
    \STATE Reduce node priority
    \STATE Avoid node in routing decisions
\ELSE
    \STATE Mark node as trusted
\ENDIF

\STATE Perform lightweight authentication

\IF{credentials are valid}
    \STATE Generate session key
    \STATE Encrypt outgoing data packets
\ELSE
    \STATE Reject communication request
    \STATE Log unauthorized access
    \STATE \textbf{continue} to next node
\ENDIF

\STATE Evaluate routing cost function

\IF{secure and optimal path available}
    \STATE Select routing path
    \STATE Forward packets toward gateway
\ELSE
    \STATE Recompute alternative route
\ENDIF

\STATE Monitor packet delivery status

\IF{packet loss detected}
    \STATE Trigger route adaptation
...
The dataset is preprocessed before training the intrusion detection model:
 ...
 Feature normalization is applied using min-max, scaling
 Data is labeled as normal or malicious,
...
\noindent\includegraphics[width=0.45\textwidth]{figures/throughput_graph.png}



The throughput results indicate that the proposed cross-layer framework maintains stable communication performance across different traffic flows, with values ranging between 55 kbps and 62 kbps. A slight increase is observed at higher flows due to packet aggregation and efficient routing decisions. This demonstrates that the proposed system effectively handles increased traffic without significant performance degradation, ensuring reliable data transmission in wireless IoT environments.
As shown in Figure the throughput is computed using Equation (\ref{eq:throughput}).

\vspace{3mm}

\textbf{Packet Delivery Ratio Analysis}

\noindent\includegraphics[width=0.45\textwidth]{figures/pdr_graph.png}

The packet delivery ratio remains within the range of 0.72 to 0.78 across most flows, indicating high communication reliability. Minor variations are observed under higher load conditions due to network congestion. The results confirm that the integration of lightweight authentication and adaptive routing mechanisms improves packet delivery efficiency compared to conventional approaches (\ref{eq:pdr}).

\vspace{3mm}

\textbf{Packet Loss Analysis}

\noindent\includegraphics[width=0.45\textwidth]{figures/packet_loss_graph.png}

The packet loss ratio varies between 23\% and 28\%, with slightly higher loss observed during peak traffic conditions. The proposed framework effectively reduces packet loss by combining cross-layer monitoring and adaptive routing strategies. This indicates improved congestion handling and better utilization of network resources in comparison to traditional IoT communication models (\ref{eq:plr})..

\vspace{3mm}

\textbf{Transmission vs Reception Analysis}

\noindent\includegraphics[width=0.45\textwidth]{figures/tx_rx_graph.png}

The transmission versus reception graph shows a consistent difference between transmitted and received packets due to interference and channel conditions. However, the gap remains controlled, indicating efficient packet forwarding and minimal data loss. This validates the effectiveness of the proposed routing and security mechanisms in maintaining communication reliability.(\ref{eq:pdr}).

\vspace{3mm}

\textbf{Throughput Distribution Analysis}

\noindent\includegraphics[width=0.45\textwidth]{figures/throughput_distribution.png}

The throughput distribution across nodes remains consistent at approximately 18 kbps, indicating uniform load balancing and efficient resource utilization. Minimal variation between nodes demonstrates that the proposed framework ensures fairness in data transmission across the network.
(\ref{eq:throughput_phase3}).

\vspace{3mm}

\textbf{Reliability Analysis}

\noindent\includegraphics[width=0.45\textwidth]{figures/reliability_pdr.png}

The reliability values remain in the range of 0.72 to 0.78 across different flows, confirming stable network performance under varying conditions. This highlights the effectiveness of the proposed cross-layer design in maintaining high reliability while managing network dynamics.
(\ref{eq:plr}).
...
\noindent\includegraphics[width=0.45\textwidth]{figures/throughput_comparison.png}

The proposed model achieves a throughput of approximately 60 kbps, significantly outperforming existing approaches such as \cite{mustafa2024framework} and \cite{khalil2024connectivity}, which report throughput values in the range of 16–18 kbps. This improvement is attributed to efficient cross-layer coordination and optimized routing mechanisms that enhance data transmission efficiency.
(\ref{eq:rec}).
...
\noindent\includegraphics[width=0.45\textwidth]{figures/pdr_comparison.png}

The packet delivery ratio of the proposed system is approximately 0.75, which is higher than the values reported in existing IoT frameworks such as \cite{sreevidya2024trust} and \cite{xiao2024multipath}. These models rely on trust-based routing and multipath communication but lack integrated cross-layer optimization, resulting in comparatively lower reliability.

\vspace{3mm}

\textbf{Packet Loss Comparison}

\noindent\includegraphics[width=0.45\textwidth]{figures/plr_comparison.png}

The packet loss ratio of the proposed framework is reduced to approximately 25\%, compared to higher loss rates in traditional models. Existing approaches such as \cite{sreevidya2024trust} and \cite{xiao2024multipath} experience higher packet loss due to limited coordination between security and routing mechanisms. The proposed model improves performance through adaptive cross-layer control.

\vspace{3mm}

\textbf{Latency Comparison}

\noindent\includegraphics[width=0.45\textwidth]{figures/latency_comparison.png}

The latency of the proposed system is approximately 30 ms, which is lower than the delay observed in existing models such as \cite{musaddiq2020rl} and \cite{khalil2024connectivity}, where latency ranges between 35–40 ms. The reduction in delay is achieved through optimized routing decisions and efficient packet processing mechanisms.

\vspace{3mm}

\textbf{Energy Consumption Comparison}

\noindent\includegraphics[width=0.45\textwidth]{figures/energy_comparison.png}

The energy consumption of the proposed framework is approximately 0.8 J, which is significantly lower than existing models such as \cite{mustafa2024energy} and \cite{musaddiq2020rl}, where energy consumption ranges between 1.0–1.2 J. This demonstrates that the proposed cross-layer approach effectively reduces energy usage while maintaining high security and performance.

\vspace{3mm}

\textbf{Accuracy Comparison}

\noindent\includegraphics[width=0.45\textwidth]{figures/accuracy_comparison.png}

The intrusion detection accuracy of the proposed system reaches approximately 96\%, outperforming existing machine learning-based IoT security models such as \cite{mustafa2025ml} and \cite{altalhan2025imbalance}, which achieve accuracy levels between 88–94\%. This improvement is due to the use of cross-layer feature extraction and adaptive learning mechanisms that enhance detection capability.
